...
const mockDate = customDate => class extends Date { //nowa klasa mockDate (jest funkcją, która zwraca klasę)
  //- działa dokladnie tak samo jak Date. Można zdefiniować anonimową klasę i przypisać ją do zmiennej lub stałej.
  //Dzięki temu dbamy o zakres zmiennych – nasza klasa mockDate ma istnieć tylko w tym bloku describe.
  constructor(...args) {
...
const checkDescriptionAtDay = (day, expectedDescription) => {//checkDescriptionAtDaysToSummer zawiera definicję testu, wykorzystującego
...
const checkDescriptionAfterDay = (day, delayDays, expectedDescription) => {
  it(`should show correct value ${delayDays} days after ${day}`, () => {
    ...
    jest.advanceTimersByTime(delayDays * 1000 *60 * 60 * 24); //Nasz komponent co sekundę sprawdza aktualny czas (za pomocą Date) i na
    //jego podstawie wyświetla odpowiednią wartość. Oznacza to, że co sekundę komponent na nowo się renderuje. Za pomocą metody
    //advanceTimersByTime przyspieszamy bieg czasu właśnie po to, aby wykonało się kolejne odświeżenie komponentu.
